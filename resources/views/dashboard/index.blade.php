        ...
        <div class="grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-4 mb-7">
            ...
            <div class="card border-l-4 border-pink-400 p-4">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-[11px] font-bold tracking-widest uppercase text-[var(--text-secondary)]">Saldo Bersih</span>
                    <span class="w-[34px] h-[34px] rounded-[10px] bg-[var(--pink-light)] flex items-center justify-center text-[var(--pink-dark)] shrink-0">
                ...
                <div class="text-xs text-[var(--text-secondary)]">{{ $balance >= 0 ? '+' : '-' }} Bulan {{ now()->isoFormat('MMMM Y') }}</div>
            </div>
            ...
            <div class="card border-l-4 border-green-500 p-4">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <span class="w-[34px] h-[34px] rounded-[10px] bg-green-50 flex items-center justify-center text-green-700 shrink-0">
            ...
            <div class="card border-l-4 border-rose-500 p-4">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-[11px] font-bold tracking-widest uppercase text-[var(--text-secondary)]">Pengeluaran</span>
                    <span class="w-[34px] h-[34px] rounded-[10px] bg-rose-50 flex items-center justify-center text-rose-600 shrink-0">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5 mb-5">
            ...
            <div class="card p-4">
                <div class="flex items-center justify-between mb-5 flex-wrap gap-2">
                    ...
                    <a href="{{ route('transactions.index') }}" class="text-[13px] text-[var(--pink-dark)] font-semibold no-underline">Lihat Semua →</a>
                </div>
                    ...
                    @forelse($transactions as $tx)
                        <div class="flex items-center gap-3.5 px-2.5 py-2.5 rounded-xl transition-colors hover:bg-gray-50
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-sm font-semibold text-[var(--text-primary)] whitespace-nowrap overflow-hidden text-ellipsis">
                                    {{ $tx->description }}
                                </div>
                            ...
                            <div class="text-right shrink-0 flex flex-col items-end gap-1
                                ...
                                <div class="text-sm font-bold whitespace-nowrap {{ $tx->type === 'income' ? 'text-green-700' : 'text-rose-600' }}">
            ...
            {{-- Right Side Column --}}
            <div class="flex flex-col gap-5 max-[992px]:grid max-[992px]:grid-cols-[repeat(auto-fit,minmax(280px,1fr))] max-[576px]:grid-cols-1">
                    ...
                    @forelse($expenseByCategory as $cat)
                        <div class="mb-3.5">
                            <div class="flex items-center justify-between mb-1.5 gap-2">
                                <span class="text-[13px] text-[var(--text-primary)] flex items-center gap-2 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap">
                ...
                <div class="card p-4">
                    <div class="flex items-center justify-between mb-4 gap-2">
                        ...
                        <a href="{{ route('banks.index') }}" class="text-xs text-[var(--pink-dark)] font-semibold no-underline">Kelola →</a>
                    </div>
                    <div class="flex flex-col gap-2.5">
                        @forelse($banks as $bank)
                            <div class="flex items-center gap-3 px-3 py-2.5 rounded-xl min-w-0"
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="text-[13px] font-semibold text-[var(--text-primary)] overflow-hidden text-ellipsis whitespace-nowrap">
                                        {{ $bank->name }}
                                    </div>
                ...
                <div class="card p-4">
                    <div class="flex items-center justify-between mb-4 gap-2">
                        ...
                        <a href="{{ route('targets.index') }}" class="text-xs text-[var(--pink-dark)] font-semibold no-underline">Kelola →</a>
                    </div>
                    <div class="flex flex-col gap-2.5">
                        @forelse($targets as $target)
                            <div class="p-3 rounded-xl" style="background:{{ $target->color }}10; border:1px solid {{ $target->color }}25;">
                                <div class="flex items-center gap-2 mb-2">
                                    <span class="shrink-0">{{ $target->icon }}</span>
                                    <span class="text-[13px] font-semibold text-[var(--text-primary)] flex-1 overflow-hidden text-ellipsis whitespace-nowrap">
                                        {{ $target->name }}
                                    </span>
                                ...
                                <div class="h-1.5 rounded-full bg-slate-100 mb-1.5">
                                    <div class="h-1.5 rounded-full" style="width:{{ $target->progress_percent }}%; background:{{ $target->color }};"></div>
                                </div>
                                <div class="text-[11px] text-[var(--text-secondary)] overflow-hidden text-ellipsis whitespace-nowrap">
